...
- **Timeout with `[RPTK][flush]`:** keep `rptk_socket.lua` beside the host
  modules. Reaper's Windows LuaSocket can report zero progress when indexed
  `send()` receives a large original string, so RPTK must send actual small
  chunks.
